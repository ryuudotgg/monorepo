...
export type UploadReturnType = string | { id: string | number; src: string };
...
function handleDataUrl(src: string): { blob: Blob; extension: string } {
    ...
    return {
      setImages:
        ...
        ({ commands }) => {
          ...
          if (errors.length) this.options.onValidationError?.(errors);
